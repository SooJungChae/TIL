...
실행 컨텍스트 종류에는 
맨 처음 생성되는 "글로벌 실행 컨텍스트", 함수가 호출될 때마다 생성되는 "함수 실행 컨텍스트" 가 있다.

이 컨텍스트들은 모두 '생성' 되고 '실행' 하는 단계를 거친다.

'생성' 단계에서는 window, this 가 생성되고, 변수가 선언되고, 기본적으로 undefined 가 할당된다.
그리고 선언식 함수는 메모리에 올라간다. 
만약 선언할 변수나 함수가 없다면 메모리공간 확보나 변수선언에서 호이스트를 할 필요가 없다.

(이렇게 생성단계에서 변수의 선언시 undefined 로 값이 초기화되는 것을 Hoisting 이라고 한다.)

'실행' 단계에 접어들면, 자바스크립트 엔진이 코드를 한줄씩 실행한다.
그리고 진짜 값을 이미 메모리에 있는 변수에 할당한다.

(자바스크립트 엔진이 코드를 한 줄씩 해석하기 시작했을 때는 이미 생성단계가 발생한 이후다.!)

함수가 호출이 되면 "함수 실행 컨텍스트"가 만들어지면서 내부에 작성된 함수가 실행된다.

함수가 모든 동작을 끝마치면 실행스택에서 해당 실행 컨텍스트가 사라지게 된다. 
그래서 함수 내부에서 생성된 변수는 내부에서만 접근이 가능하고, 함수 실행 컨텍스트가 실행 스택에서 제거되면 그 변수에 접근할 수 없다.

이 예외는 '클로저'를 사용하면 예외의 상황을 만들 수 


### 글로벌 실행 컨텍스트
- 자바스크립트 엔진이 처음으로 코드 해석을 시작할 때 생성된다.
- **global object**, this, 변수와 함수를 위한 메모리 공간 확보
- 선언식 함수를 메모리에 배치하면서 변수 선언에 기본값 undefined를 할당

### 함수 실행 컨텍스트
- 함수가 호출될 때마다 생성된다.
- **arguments object**, this, 변수와 함수를 위한 메모리공간 확보
- 내부 변수가 없으면 메모리에 할당되지 않는다.
- 선언식 함수를 메모리에 배치하면서 변수 선언에 기본값 undefined를 할당
...
동적 스코프와 렉시컬 스코프가 있는데,
거의 대부분의 언어가 "렉시컬 스코프" 를 사용한다.

자바스크립트가 파싱을 하면서 변수를 처리해야하기 때문에 
변수가 소스코드 내 어디에서 선언되었는지 범위를 지정하는 것을 "렉시컬 스코프" 라고 한다. 
 
> 렉시컬 스코프: 함수를 **어디에 선언**했는지에 따라 상위 스코프를 결정하는 것 

- [스코프](https://developer.mozilla.org/ko/docs/Web/JavaScript/Closures)
