...
콜스택에 비운다. 그 동안 자바스크립트 엔진은 스택에 담긴 스크립트들을 처리한다.

Web API 에서 작업이 끝나게 되면

브라우저에서 event loop 가
크롬의 경우 브라우저 내부에 V8 엔진이 있는데 엔
동기적으로 비동기적으로  자바스크립트 코드를 만나면 크롬의 V8 같은 자바스크립트 런타임과 브라우저가 이벤트를 처리하는 방식
 
stack : 브라우저의 자바스크립트 V8 엔진안에 있는 메모리 관리 . 함수가 호출되면 stack 에 함수가 추가된다.
함수에서 리턴이 일어나면 스택에서 종료시킨다. (함수를 꺼낸다.)

Web API 함수를 만나면 바로 종료시키고 Web API 처리기에 집어넣는다. 
동작이 다 끝나면 task que 에 들어가게 된다.

이벤트 루프는 이 stack 과 task que 를 지켜보고 있다.

task que 에 처리할게 있으면 stack 이 빌때까지 기다리고
stack 이 비면 stack 에 넣어준다.





render 는 콜백에 비해 더 높은 우선순위를 갖는다. 
매 16밀리세컨마다 큐에 렌더가 들어간다. 스택이 깨끗해진 후에야 렌더링을 한다.
callback que 에 할일이 많아지면 렌더링을 못하게 되는 것이다. 

스크롤 될 경우 15밀리세컨마다 작동한다고 하면 callback que 에 쌓인 애들을 처리할 동안 렌더링은 못하게 된다. 1초에 
