...
> '싱글 스레드' 기반의 자바스크립트가 '동기성' 을 지원하는 방식
> 자바스크립트 코드를 만나면 크롬의 V8 같은 자바스크립트 런타임과 브라우저가 이벤트를 처리하는 방식

자바스크립트 엔진은 한 번에 한가지 일만 처리할 수 있는 '싱글 스레드' 이다. 
이런 자바스크립트가 브라우저서 멀티스레드 처럼 보여지는 이유는 브라우저 엔진의 '이벤트 루프' 덕분이다.
...
크롬에서 자바스크립트 작업을 처리하는 곳은 V8 엔진의 '콜스택' 영역이다.
함수가 호출될 경우 순차적으로 콜스택에 들어오게 되고, 
콜스택이 비워질 때까지 처리하게 된다.

만약, `console.log` 나 `setTimeout` 과 같은 함수가 들어오게 된다면,
이것들은 자바스크립트가 아니라 Web API 에서 처리하는 작업임으로 Web API 에게 처리해달라고 부탁하고
콜스택에 비운다. 그동안 콜스택에 추가된 다른 작업들을 처리한다.

이벤트 루프는 call stack 과 task que 를 계속해서 주시하고 있는데, 
call stack 의 영역이 비워질 때까지 기다리고 비워지면 task que 에 처리할 게 남아있는지 확인한다.

Web API 에서 작업이 끝나게 되면 콜백함수가 Task que 에 추가된다.
task que 를 지켜보고 있던 이벤트 루프는 call stack 에 처리할 작업이 있는지 확인한 후
없으면 task que 에서 일을 가져와 call stack 에서 처리하게 된다.

이런 과정이 있기 때문에 화면을 그릴 때 좀 느리거나 멈추게 되는 '블로킹' 현상이 일어나게 된다.
(* 블로킹 : 느린 동작이 스택에 남아서 멈추는 현상)

tack que 에는 브라우저에 화면을 그리는 render que 과 콜백 함수가 들어오는 callback que 가 있다.
(매 16밀리세컨마다 큐에 렌더가 들어간다.)

render 는 callback에 비해 더 높은 우선순위를 갖기 때문에 call stack 이 비워졌을 때 render task 가 먼저 들어가는데,
그 이후에 처리할 작업이 많은 callback 함수가 들어가게 되면 렌더링 시간은 느려지고 멈추게 되는 것이다.
 
stack : 브라우저의 자바스크립트 V8 엔진안에 있는 메모리 관리 . 함수가 호출되면 stack 에 함수가 추가된다.
함수에서 리턴이 일어나면 스택에서 종료시킨다. (함수를 꺼낸다.)

예를들어, 스크롤이 될 경우 15밀리세컨마다 작동하는데 
callback que 에 쌓인 애들을 처리할 동안 렌더링은 못하게 된다.
callback 함수가 많은 작업을 수행해야 한다고 하면 렌더링 되는 시간은 더욱 길어질 것이다.

이런 블로킹을 해결할 수 있는 방법은 무엇이 있을까 생각해보면 *성능 최적화*에 대해 고민하게 된다.

- web worker
- image render 작업

왜 이벤트 루프 처리 방식을 이해하고 자바스크립트 처리 시간을 줄여야 하는지 이해가 됐다 ...
이전에 화면 구동이 너무 느려서 답답했는데, 현상을 이해 하지 못하고 해결해놓지 못해서 아쉽다. 
